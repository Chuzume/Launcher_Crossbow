...
# 減算
    #execute store result score $Ammo2 Chuz.Temporary run data get storage chuz:context Item.Mainhand.Count
    #scoreboard players operation $Ammo2 Chuz.Temporary -= $1 Chuz.Temporary
...
# サウンド
    execute at @s run playsound minecraft:block.stone_button.click_on player @a ~ ~ ~ 1 1.75
    execute at @s run playsound minecraft:block.note_block.xylophone player @a ~ ~ ~ 1 2
    execute at @s run execute anchored eyes run particle crit ^ ^ ^1 0 0 0 0.5 2