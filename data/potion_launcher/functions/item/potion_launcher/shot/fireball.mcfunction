...
# 目線の高さに召喚
    execute anchored eyes run summon small_fireball ^ ^ ^ {Fire:2,Tags:["Pot.L_ProjectileInit"]}

# サウンド
    stopsound @s player minecraft:item.crossbow.shoot
    playsound minecraft:entity.blaze.shoot player @a ~ ~ ~ 2 1
    playsound minecraft:entity.arrow.shoot player @a ~ ~ ~ 2 0.5
    playsound minecraft:item.crossbow.shoot player @a ~ ~ ~ 2 1.5

# 弾丸のステータス用のスコアを設定する
    scoreboard players set @s Pot.L_Life 200
    scoreboard players set @s Chuz.Speed 30

# Init処理
    execute as @e[type=small_fireball,tag=Pot.L_ProjectileInit] at @s rotated as @p[tag=Chuz_This] run function potion_launcher:entity/projectile_common/init
